...
spectral dataset/library was used in the \sQuote{Chimiometrie 2006} challenge by @pierna2008soil. The library contains NIR absorbance spectra of dried and sieved
825 soil observations/samples. These samples originate from agricultural fields collected from all over the Walloon region in Belgium. The data are in an `R` `data.frame` object which is organized as follows:
...
NIR absorbance spectra of the samples recorded between the 
...
functions wthin the package (see `f_diss()`, `cor_diss()`, `sid()` and 
...
To measure $d$ between two observations $x_i$ and $x_j$ 
