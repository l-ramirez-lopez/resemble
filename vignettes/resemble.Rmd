...
Modelling using spectral data has garnered wide interest in the last two decades. Spectroscopy is the study of the spectral signature of material (e.g. soil, leaf, etc.). The spectral signature directly or indirectly relates to a wide range of chemical, physical of biological characteristics of the material under study. Spectroscopic measurements are fast, non-destructive and cost-efficient. The wide interest of spectroscopy has led to recent initiatives compiling large amount of spectral data, which increases *de facto* the complexity of the resulting datasets. To analyse large and complex spectral data, one must then resort numerical and statistical tools such as dimensionality reduction, and local spectroscopic modelling based the most similar spectra found in the dataset.    

The `resemble` package contains functions for dimensionality reduction, computing spectral dissimilarity matrices, nearest neighbour search, and to fit local spectroscopic models. The core package functionalities include: 
...
__Example dataset__
The example of this vignette uses external spectroscopic data. The spectral data are scans of soil samples recorded in the the visible and near-infrared (vis-NIR) range of the electromagnetic spectrum.
[here data description]

Load the necessary packages and data. [to be improve - maybe best separate package loading and data description]
...
[AW: When I have the data I can fill all this]
...
Explain briefly how to do it and how it is. Mention that openMP is also used in cpp code
