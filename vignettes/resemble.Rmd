...
This vignette uses a soil spectroscopic dataset provided in the `resemble`
package. This data is stored in a data frame which includes a total of 772 
observations, of which 742 come from a soil spectral library of Australia 
(covering Queensland, New South Wales, South Australia, Victoria and Western 
Australia, @rossel2010using). The remaining 30 observations come from a small 
local farm also in Australia within the region covered by the spectral library. 

In the data frame, each observation/soil sample has an unique identifier (ID), 
a reference value of clay content, the set it originates from (i.e. Australia's
spectral library or local farm) and its corresponding spectrum. The spectra is
stored as a matrix inside the data frame (accessed with `$spc`). Each column in 
this matrix represents a spectral variable, the `colnames` of this matrix 
indicate the wavelengths of the spectra. The spectral data is recorded as 
absorbance in the visible and near-infrared (vis-NIR) range of the electromagnetic 
spectrum between 351 and 2500 nm at 7 nm resolution.
...
wavs <- as.numeric(colnames(SoilSpectroscopy$spc))
...
For the examples presented here, the SoilSpectroscopy data is split into 
training (75%, of the Autralia library), testing (25%, of the Autralia library) 
and validation subests (). 
