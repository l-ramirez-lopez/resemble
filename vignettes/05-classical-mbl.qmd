...
format:
  html:
    ...
    smooth-scroll: true
vignette: >
  %\VignetteIndexEntry{Classical memory-based learning (local modelling)}
...
colnames(my_tab) <- c("Abbreviation",
...
```{r cec-examples, results = 'hide', eval = TRUE, fig.show = 'hide'}
# Let's define some methods:
my_wapls <- fit_wapls(min_ncomp = 2, max_ncomp = 25)
...
# Use one where PCA dissimilarity is used and the dissimilarity matrix
# is used as source of additional predictors
# let's define first an appropriate vector of dissimilarity thresholds
...
# Use one where PLS dissimilarity is used and the dissimilarity matrix
# is used as source of additional predictors
pls_pred_cec <- mbl(
  Xr = train_x[!is.na(train_cec), ],
  Yr = train_cec[!is.na(train_cec)],
...
# Use one where Gaussian process regression and PCA dissimilarity are used
...
```{r collect-predictions, eval = TRUE, fig.show = 'hide'}
...
preds <- cbind(
  get_predictions(local_cec)[, ..bi_local],
...
The scatter plots below illustrate the prediction results obtained for CEC with each of the MBL configurations tested.

```{r cec-plots-code, eval = FALSE, fig.show = 'hide'}
old_par <- par("mfrow", "mar")

par(mfrow = c(2, 2))
plot(test_cec, preds[, 2],
     xlab = "Predicted CEC, meq/100g",
     ylab = "CEC, meq/100g", main = colnames(preds)[2])
abline(0, 1, col = "red")

plot(test_cec, preds[, 3],
     xlab = "Predicted CEC, meq/100g",
     ylab = "CEC, meq/100g", main = colnames(preds)[3])
abline(0, 1, col = "red")

plot(test_cec, preds[, 4],
     xlab = "Predicted CEC, meq/100g",
...
```{r mblcomparisons, fig.cap = "CEC prediction results for the different MBL configurations tested", fig.align = "center", fig.width = 8, fig.height = 8, echo = FALSE}
old_par <- par("mfrow", "mar")
...
# R2s
r2s <- round(cor(test_cec, preds, use = "complete.obs")^2, 2)
...
If information of the response values in the prediction set is available, then, the `Yu` argument can be used to directly validate the predictions done by `mbl()`. It is not taken into account for any optimization or modeling step. It can be used as follows:

```{r yu-argument, eval = FALSE}
# Use Yu argument to validate the predictions
pc_pred_nt_yu <- mbl(
  Xr = train_x[!is.na(train_nt), ],
  Yr = train_nt[!is.na(train_nt)],
  Xu = test_x,
  Yu = test_nt,
...
The `mbl()` function uses the `foreach()` function of the [foreach package](https://CRAN.R-project.org/package=foreach) for iterating over every row/observation passed to the argument `Xu`. In the following example, we use the [doParallel package](https://CRAN.R-project.org/package=doParallel) to set up the cores to be used. Alternatively the [doSNOW package](https://CRAN.R-project.org/package=doSNOW) can also be used. In the following example we use parallel processing to predict Total Nitrogen:
