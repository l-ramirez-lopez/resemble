...
dissimilarity <- function(Xr, 
  ...
  if(!diss_method %in% avalmethods){
    stop(paste("'diss_method' argument needs to be equal to one of the following options:\n", 
  ...
  if(gh){
    if(diss_method == "pls"){
      ...
      pca <- pls_projection(Xr = rbind(Xr, Xu), 
                           Xu = NULL, 
                           Yr = c(Yr, 
