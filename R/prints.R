...
.col_blue <- function(x) paste0("\033[34m", x, "\033[39m")
.col_bold_red <- function(x) paste0("\033[1;31m", x, "\033[0m")
...
print.liblex <- function(x, ...) {
  ...
  if (!is.null(x$optimal_params)) {
    ...
    cat("  ncomp:", x$optimal_params$ncomp["min"], "-", 
        x$optimal_params$ncomp["max"], "\n")
