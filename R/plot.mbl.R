...
#' see details).
#' @param param one of the following options "rmse", "st_rmse" or "r2". The 
#' respective validation statistic is then plotted. It is only available if 
#' \code{"validation"} is specified in the \code{g} argument.
#' @param pls_c a vector of length one or two indicating the pls factors to be 
...
plot.mbl <- function(x,
  ...
  if ("validation" %in% g) {
    ...
    if (is.null(tpl)) {
      ...
      to_plot <- tpl %>% select(names(tpl)[dt]) %>% reshape(timevar = "val", 
                                                            idvar = idv, 
                                                            direction = "wide")
      ...
      do.call("matplot", c(list(x = to_plot[, 1], 
                                y = to_plot[, -1],
                                type = "b", 
                                xlab = dtn[1],
                                ylab = param, 
  ...
  if ("gh" %in% g & !is.null(x$gh)) {
    ...
    if (object$gh$projection$n_components == 1) {
      ...
      tp <- data.frame(index = 1:length(tp), tp = tp, 
                       set = c(rep("Xr", nrow(xr_scores)), 
                               rep("Xu", nrow(xu_scores))))
      tp <- tp[order(tp$tp), ]
      tp$index <- 1:length(tp$index)
      
      do.call("plot", c(list(x = tp[tp$set == "Xr", 1], 
                             y = tp[tp$set == "Xr", 2],
                             ylim = rng,
                             col = xr_col,
      ...
      do.call("plot", c(list(x = xr_scores[, pls_c[1]], 
                             y = xr_scores[, pls_c[2]],
                             xlab = xl, 
                             ylab = yl, 
                             xlim = rng, 
