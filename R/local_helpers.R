...
get_neighbor_info <- function(Xr, Xu, diss_method, Yr = NULL, 
  ...
  info_neighbors <- do.call(search_neighbors, 
                            c(list(Xr = Xr, Xu = Xu, diss_method = diss_method, 
                                   Yr = Yr, 
                                   k = k_max, 
  ...
  if (diss_usage == "predictors") {
    ...
    if (diss_method %in% c("pca", "pca.nipals", "pls") ) {
      ...
      info_neighbors$diss_xr_xr <- dissimilarity(scale(rbind(Xr, Xu), 
                                                       center = center,
                                                       scale = scale)[1:nrow(Xr), ],
                                                 diss_method = diss_method, 
                                                 center = FALSE, 
                                                 scale = FALSE, ...)$dissimilarity
    }
  }
...
sample_str <- function(y, p, number, group = NULL) {
  ...
  if (is.null(group)) {
    nv <- floor((1 - p) * nrow(y))
    nv <- ifelse(nv < 1, 1, nv)
    y_strata <- unique(quantile(y, probs = seq(0, 1, length = (nv + 1)), 
                                names = FALSE))
    y_cuts <- cut(y, 
                  breaks = y_strata,
                  labels = 1:(length(y_strata) - 1),
                  include.lowest = TRUE)
    strata_category <- data.table(original_order = 1:length(y), 
                                  strata = y_cuts)
    
    hold_out <- matrix(NA, nv, number)
    hold_in <- matrix(NA, nrow(y) - nv, number)
    colnames(hold_out) <- colnames(hold_in) <- paste0("Resample_", 
                                                      seq(1:number))
    ...
    for (jj in 1:number) {
      strata_samples <- sort(tapply(X = strata_category$original_order,
                                    FUN = function(x) {
                                      if (length(x) == 1) {
      ...
      if (length(strata_samples) < nv) {
        adds <- sample((1:length(y))[-strata_samples], 
    ...
    # vector of means
    aggregated_y <- tapply(X = y_groups$y, 
    ...
    y_cuts <- cut(aggregated_y, 
                  ...
                  include.lowest = TRUE)
    strata_category <- data.table(original_order = 1:length(aggregated_y), 
    ...
    for (jj in 1:number) {
      ...
      if (length(strata_samples) < nv) {
        adds <- sample((1:nrow(strata_category))[-strata_samples], 
                       nv - length(strata_samples))
        strata_samples <- c(strata_samples, adds)
      }
...
pls_cv <- function(x, y, ncomp,
  ...
  if (method == "wapls" & retrieve & tune) {
    ...
    search_grid <- expand.grid(min_component = seq_pls, 
  ...
  if (method == "pls") {
    val$cv_results <- data.table(npls = 1:ncomp, 
  ...
  if (method == "wapls" & retrieve & !tune) {
    ...
    val$cv_results <- data.table(min_component = min_component, 
                                 max_component = ncomp, 
  ...
  if (method == "wapls" & retrieve & tune) {
    val$cv_results <- data.table(search_grid, rmse_cv = rowMeans(cv_results$rmse_seg), 
                                 st_rmse_cv = rowMeans(cv_results$st_rmse_seg), 
                                 rmse_sd_cv = get_col_sds(t(cv_results$rmse_seg)), 
                                 r2_cv = rowMeans(cv_results$rsq_seg))
...
ith_mbl_neighbor <- function(Xr, Xu, Yr, Yu = NULL, 
  ...
  nextEl <- function() {
    ...
    it_neigh <- list(ith_xr = ith_xr_neighbors,
...
get_ith_local_neighbors <- function(ith_xr, ith_xu, ith_yr, ith_yu = NULL,
  ...
  local_diss <- do.call(ortho_diss, 
                        c(list(Xr = ith_xr, Xu = ith_xu,
                               Yr = ith_yr,
                               pc_selection = pc_selection,
                               diss_method = diss_method,
                               center = center,
  ...
  if (is_predictors) {
    ...
    colnames(ith_local_xr_xr_diss) <- paste0("k_", 
                                             1:ncol(ith_local_xr_xr_diss))
    ith_xr <- cbind(ith_local_xr_xr_diss, 
...
gaussian_pr_cv <- function(x, 
  ...
  val <- NULL
  val$resamples <- cv_samples$hold_out
  val$cv_results <- data.table(rmse_cv = mean(cv_results$rmse_seg), 
                               st_rmse_cv = mean(cv_results$st_rmse_seg), 
                               rmse_sd_cv = sd(cv_results$rmse_seg), 
                               r2_cv = mean(cv_results$rsq_seg))
  ...
  if (retrieve == "final_model") {
    ...
    val$model <- gaussian_process(X = x, 
                                  Y = y, 
                                  noisev = noise_variance, 
                                  scale = scale)
  }
