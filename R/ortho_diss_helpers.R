...
local_ortho_diss <- function(k_index_matrix, Xr, Yr, Xu, 
  ...
  if (n_xr + n_xu == ncol(k_index_matrix)) {
    ...
    diag(distnc) <- 0
    ith_subsets <- ith_subsets_ortho_diss(x = Xr, y = Yr, 
    ...
    ith_subsets <- ith_subsets_ortho_diss(x = Xr, xu = Xu, y = Yr, 
                                          kindx = k_index_matrix, 
                                          na_rm = TRUE)
    n_rows_d <- n_xr
  ...
  local_d <- foreach(
    ...
    ith_projection <- ortho_projection(Xr = ith_local_subset$x, 
    ...
    ith_dist <- dist_vec
    ith_dist[3:length(ith_dist)] <- f_diss(Xr = scores[1, , drop = FALSE],
  ...
  vec_positions <- sweep(k_index_matrix, MARGIN = 2, 
                         STATS = add_row_indices, FUN = "+")
  distnc[vec_positions] <- as.vector(local_d)
  return(list(dissimilarity_m = distnc,
...
format_xr_xu_indices <- function(xr_xu_names){
  ...
  xu_mss <- paste0(c(if_else(length(xu_insufficient) > 0, 
                            "\nXu: ", ""), 
                     paste(xu_insufficient, collapse = ", ")), 
                   collapse = " ")
  xr_mss <- paste0(c(if_else(length(xr_insufficient) > 0, 
                            "\nXr: ", ""), 
                     paste(xr_insufficient, collapse = ", ")), 
                   collapse = " ")
  
  return(list(xr_mss = xr_mss,
              xu_mss = xu_mss))
