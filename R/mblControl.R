...
#' @description
#' This function is used to further control some aspects of the memory-based learning process in the \code{mbl} function.
#' @usage 
#' mblControl(sm = "pc",
...
#'        }
#'        The \code{"pc"} spectral dissimilarity metric is the default. If the \code{"sidD"} is chosen, the default parameters of the \code{sid} function are used however they can be modified by passing them as additional arguments (\code{...}) in the \code{\link{mbl}} function.  
#'        
#'        This argument can also be set to \code{"none"}, in such case, a dissimilarity matrix must be specified in the \code{dissimilarityM} argument of the \code{\link{mbl}} function.
#' @param pcSelection  a list specifying the details of the method to be used for identifying the number of components to be retained for computing the dissimilarities between samples when the method passed to \code{sm} is one of the following options: \code{"pc"}, \code{"loc.pc"}, \code{ "pls"} or \code{ "loc.pls"}. This list must contain two elements in the following order: \itemize{
...
#'        }}
#'        The default method for the \code{pcSelection} argument is \code{"opc"} and the maximum number of principal components to be tested is set to \code{min(dim(Xr), 40)}, where code{Xr} is the matrix of reference predictors in the \code{\link{mbl}} function.
...
#' @param ws an optional odd integer value which specifies a window size when the correlation dissimilarity is used (i.e \code{sm = "cor"}). If not specified, the standard correlation dissimilarity is computed (i.e no moving window). Default is \code{NULL}.
...
#' @param scaled a logical indicating whether or not the predictor variables must be scaled to unit variance at each local segment (before regression).
...
#' @param number an integer indicating the number of resampling iterations at each local segment when \code{"loc_crossval"} is selected in the \code{valMethod} argument. Default is 10.
...
mblControl <- function(sm = "pc",
  ...
  if(sm %in% c("loc.pc", "loc.pls")){
    ...
    if(k0 < 10 | length(k0) != 1) 
      stop(paste("'k0' must be an positive integer value larger than 10")) 
    smParam <- k0
  }
  ...
  if(pcSel %in% c("cumvar", "var"))
  {
    if(length(pcSelection) == 1){
      ...
      if(pcSelection$value > 1 | pcSelection$value <= 0) 
        stop(paste("When the method for 'pcSelection' is either 'var' or 'cumvar' the value in 'pcSelection' must be a number larger than 0 and below or equal to 1"))
    }
  }
