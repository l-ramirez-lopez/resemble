...
#' @param center a logical indicating whether or not the predictor variables must be centered at each local segment (before regression). For PLS methods centering is always done internally. If \code{TRUE}, a global centering is also applied globally for computing the dissimilarities.
#' @param scale a logical indicating whether or not the predictor variables must be scaled to unit variance at each local segment (before regression). If \code{TRUE}, a global scaling is also applied globally for computing the dissimilarities.
