...
local_fit_pls <- function(pls_c, pls_max_iter = 1, pls_tol = 1e-6) {
  ...
  if (length(pls_c) != 1 | !is.numeric(pls_c)) {
    stop(paste0("'pls_c' must be a single numerical ",
                "value specifiying the maximum number of pls components to ",
                "be evaluated"))
  }
  
  fit_type <- list(method = "pls",
                   pls_c = pls_c,
