#' @title A function that controls some aspects of the the \code{gesearch} function
#' @description
#'
#' This function is used to further control some aspects of the \code{gesearch} 
#' function.
#'
#' @usage
#' search_control(
...
#' @param p if \code{tune = TRUE}, a value indicating the proportion of
...
#' Default is 0.75 (i.e. 75 percent).
#' @param stagnation_limit Integer. Maximum number of consecutive 
#' iterations/generations during which the gene pool size is unchanged before 
#' early termination is triggered. This prevents infinite loops in cases where 
#' the target size cannot be reached due to no further removable samples. 
#' Default is 5.
...
## History:
## 2020.03.29 (Leo):  New fucntion to control gesearch
...
## 2025.08.18 (Leo):  new argument stagnation_limit
...
search_control <- function(
  ...
  if (
    !is.numeric(stagnation_limit) || 
    stagnation_limit != as.integer(stagnation_limit) ||
    stagnation_limit < 1
  ) {
    stop("stagnation_limit must be a positive whole number (integer)")
  }
