...
eval_multi_pc_diss <- function(scores, 
  ...
  if (ncol(side_info) == 1) {
    extract_sim_results <- function(x) {
      ...
      return(list(result = x$eval[,measure_names], 
                  measure_names = measure_names))
    }
    n_cols_results <- 1
  }else{
    extract_sim_results <- function(x) {
      measure_names <- c(paste0("rmsd_", names(x$eval[,"rmsd_Yr"])), 
                         "mean_standardized_rmsd_Yr")
      return(list(result = c(x$eval[,"rmsd_Yr"], 
  ...
  if (check_dims){
    if (nrow(side_info) != nrow(scores)) {
      stop(paste0("Number of observations in 'scores' do not match with the ", 
...
check_pc_arguments <- function(n_rows_x, n_cols_x, pc_selection, 
  ...
  if (pc_selection_method %in% c("opc", "manual")) {
    if (length(pc_selection) == 1) {
      ...
      pc_selection_checked <- list(method = pc_selection_method, 
                                   value = treshold_comp)
      
      message(paste0("Missing value in 'pc_selection', maximum components to be ", 
                     "tested automatically set to ", treshold_comp))
    } else {
      if (!is.list(pc_selection)) {
        stop(paste0("The 'pc_selection' argument must be a list in which the ",
      ...
      pc_selection_checked <- list(method = pc_selection[[1]], 
      ...
      if (pc_selection_checked$value < 2 | pc_selection_checked$value > min(n_rows_x, n_cols_x)) {
        stop(paste("The maximum number of principal components must be a value ",
  ...
  if (pc_selection_method %in% c("cumvar", "var")) {
    if (length(pc_selection) == 1) {
      if (pc_selection_method == "cumvar") {
        pc_selection_checked <- list(method = "cumvar", 
                                     value = default_max_cumvar)
        message(paste("Missing value in 'pc_selection', amount of cumulative ", 
                      "variance to be retained automatically set to 0.99 (99%)"))
      } else {
        pc_selection_checked <- list(method = "var", value = default_max_var)
        message(paste0("Since the value of the pc_selection argument is missing, ", 
      ...
      if (pc_selection_checked$value > 1 | pc_selection_checked$value <= 0) {
        stop(paste0("When the method for 'pc_selection' is either 'var' or ", 
  ...
  list(pc_selection_checked = pc_selection_checked,
       max_comp = max_comp)
}
