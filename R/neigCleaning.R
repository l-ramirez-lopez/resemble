...
neigCleaning <- function(Xr, Xu, 
  ...
  if(sm == "pc"){
    orthD <- orthoDiss(Xr = Xr, X2 = Xu, pcSelection = pcSelection, Yr, method = "pca",center = center, scaled = scaled, return.all = FALSE, call. = FALSE)
    d.mat <- orthD$dissimilarity
  }
  
  if(sm == "loc.pc"){
     orthD <- orthoDiss(Xr = Xr, X2 = Xu, pcSelection = pcSelection, Yr, method = "pca", local = TRUE, k0 = k0, center = center, scaled = scaled, return.all = FALSE, call. = FALSE)
     d.mat <- orthD$dissimilarity[ , , drop = FALSE]
  }
  
  if(sm == "pls"){
    orthD <- orthoDiss(Xr = Xr, X2 = Xu, pcSelection = pcSelection, Yr, method = "pls",center = center, scaled = scaled, return.all = FALSE, call. = FALSE)
    d.mat <- orthD$dissimilarity
  }
