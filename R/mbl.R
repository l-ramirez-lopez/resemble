...
#'  \item{Partial least squares (\code{"pls"}):}{ It uses the non-linear iterative partial least squares algorithm (nipals). The only parameter which needs to be optimized is the number of pls components. This can be done by cross-validation at each local segment.}
...
locFit <- function(x, y, predMethod, scaled = TRUE, weights = NULL, pred.new = TRUE, newdata, 
  ...
  if(predMethod == "pls"){
    ...
    if(CV)
    {
      ...
      x <- sweep(x, 1, weights, "*")   ###MODIFIED
      y <- y * weights
      fit <- ospls(X = x, Y = as.matrix(y), scale = scaled, ncomp = pls.c)
  ...
  if(predMethod == "wapls1"){
    if(CV)
    {
      ...
      x <- sweep(x, 1, weights, "*")   ###MODIFIED
      y <- y * weights
      fit <- ospls(X = x, Y = as.matrix(y), scale = scaled, ncomp = pls.c[[2]])
  ...
  if(predMethod == "wapls2"){
    ...
    if(CV)
    {
      cvVal <- plsCv(x = x, y = as.matrix(y), scaled = scaled, 
                     ncomp = pls.c[[2]], 
                     weights = weights, 
                     p = p, resampling = resampling, 
                     group = group, 
...
#' @keywords internal
plsCv <- function(x, y, ncomp, center = TRUE, scaled, weights = NULL, p = 0.75, resampling = 10, group = NULL,retrieve = c("final.model", "all.models", "none")){
...
#' @description internal
#' @keywords internal
#' @param plsO either an object returned by the \code{plsCv} function or an object as returned by the \code{ospls} function which contains a pls model.
...
wapls.weights <- function(plsO, orgX, type = c("w1", "w2"), newX = NULL, pls.c, w2_warning = TRUE){
  ...
  if(type == "w1"){
    whgt <- waplsoneweights_cpp(projectionm = plsO$projectionM, 
  ...
  if(type == "w2"){
    if(is_plsCv)
    {
      ...
      resdl <- rep(NA, maxF)
      for(i in minF:maxF){
        resdl[i] <- mean((plsO$Y - (mean(plsO$Y) + plsO$scores[,1:i,drop =FALSE] %*% plsO$Y.loadings[1:i]))^2)
      }
      resdl <- sqrt(resdl[minF:maxF])
