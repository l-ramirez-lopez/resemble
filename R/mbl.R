...
#'     ...)
#' @param Xr input \code{matrix} of predictor variables of the reference data (observations in rows and variables in columns). 
#' @param Yr a numeric \code{vector} containing the values of the response variable corresponding to the reference data
#' @param Xu input \code{matrix} of predictor variables of the data to be predicted (observations in rows and variables in columns). 
...
#' @param k a \code{vector} of inetegers specifying the sequence of k nearest neighbours to be tested. Either \code{k} or \code{k.diss} must be specified. This \code{vector} will be automatically sorted into ascending order. The values of vectors of class numeric, will be coerced to the next upper inetegers.
#' @param k.diss a numeric \code{vector} specifying the sequence of dissimilarity thresholds to be tested for the selection of the nearest neighbors found in \code{Xr} around each observation in \code{Xu}. These thresholds depend on the corresponding dissimilarity measure specified in the object passed to \code{mblCtrl}. Either \code{k} or \code{k.diss} must be specified.
#' @param k.range a \code{vector} of two integers which specifies the minimum (first value) and the maximum (second value) number of neighbours allowed when the \code{k.diss} argument is used.
