...
search_neighbors <- function(Xr, Xu, diss_method = c("pca",
  ...
  if (diss_method == "cor") {
    if (!is.null(ws)) {
      if (ws < 3 | ws > (ncol(Xr) - 1) | length(ws) != 1 | (ws %% 2) == 0) {
        stop(paste("ws must be an odd value between 3 and", ncol(Xr) - 1, 
                   "(ncol(Xr) - 1)"))
      }
    }
  }
  ...
  if (!is.null(k_diss)) {
    ...
    if (length(k_range) != 2 | !is.numeric(k_range) | diff(k_range) < 0) {
      stop(paste0("k_range must be a vector (of length 2) which specifies the ", 
    ...
    if (k_max > nrow(Xr)) {
      stop(paste0("Maximum number of nearest neighbors cannot exceed the ", 
